...
This chapter describes a model developed for a multi-modelling study. Four teams with existing experience in modeling cholera in Haiti were tasked to model scenarios of mass vaccination administration towards cholera immunity. The full model study was performed by Lee et al. and has been published as:
...
For this collaborative effort, we developed a hidden markov model of cholera transmission, with specific modeling choices to account for vaccination and to gather information on
...
\section{Cholera in Haiti}
Cholera (\textit{V. Cholerae} of serogroup O1, serotype Ogawa and biotype El Tor) was introduced in Haiti by UN soldiers. 

Cholera in Haiti has been modeled in the lab. However, for this effort.
\begin{figure*}
	\centering
...
It is the stochastic translation of a deterministic SIRB model based on ordinary differential equations which has been extensively used to simulate the Haitian cholera epidemic in previous studies\cite{Rinaldo:Reassessment20102011:2012,Bertuzzo:ProbabilityExtinctionHaiti:2016,Pasetto:RealtimeForecastingCholera:2018}. This model is described at the end of \textsc{Chapter 3}. The common outputs for the exercise are the distrivution of time to elimination and probability of elimination in different scenario, we opted for a stochastic  model based on a Partially-Observed Markov Process (POMP), simulating the stochastic transitions between compartments as discrete events\cite{King:InapparentInfectionsCholera:2008}. Other modifications ensued to adapt the model described in \textsc{Chapter 3}.
...
We present the results for six scenarios, with deployement shown in Figure\ref{fig:deploy}.
\begin{itemize}
	\item \textbf{No vaccination} status quo with no campaign.
	...
	\item 	\item \textbf{National over 5 years} 5 years national OCV campaign with the same coverage described above.
...
\end{itemize}
The estimated vaccine efficacy was waning from 76\% over 60 month, and consivertatively assumed to provide no protection after 5 years.
...
\end{figure*}
 ...
 In Haiti, there have been no laboratory-confirmed cases of cholera in Haiti from February 2019. This has sparked claims of elimination from the island and from the Americas. In our model results, the probability of elimination in the no vaccination scenario is very low. Which gring
