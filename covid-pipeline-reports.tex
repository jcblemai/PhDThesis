...
\label{sec:covid-pipeline-reports}
  \end{fullwidth}
The ongoing Coronavirus disease 2019 (COVID-19, previously nCov) pandemic originated in Wuhan, China at the end of year 2019. The pathogen here is a virus, the severe acute respiratory syndrome coronavirus SARS-CoV-2, which upon infections causes might cause symptoms accross all range of severity. With 4.07\textsc{M} confirmed deaths and 190\textsc{M} confirmed cases as of July 2021, it is one of the deadliest pandemic in history.

 The response has mobilized scientists, public-health and medical professionals and politics around the world. Over a year and half, a considerable body of evidence has accumulated\sidenote[][-3\baselineskip]{With more than 190'000 peer-reviewed papers and many more preprints, website and reports published at the time of writing. Estimation from: \fullcite{COVID-19OpenAccessProject:LivingEvidenceCOVID19:2020}}. For this reason, we do not endeavour a literature review of the current (and evolving) understanding of COVID-19, nor the modeling studies which have informed the response. 

 This chapter presents some modeling works for the response to COVID-19. While some of these work are still ongoing, we present them at an early stage, a perspective from around July 2020. The state of knowledge on the SARS-CoV-2 and COVID-19 was (and still is) quickly expanding, and the present chapter highlights the challenges of dealing with such uncertainties. 
...
The model for SARS-CoV-2 transmission is entirely configurable to any compartment model following an arbitrary transition graph, allowing for multi-strain or age-stratified models. We present here the initial model built for scenario planning, and used until June 2020. This initial model was simple spatial $S\longrightarrow E \longrightarrow I \longrightarrow R$ model. In practice, we used $k = 3$ compartments of infected, $I_1, I_2, I_3$ in order to have an infectious period shaped as Erlang distribution.

Two meta-parameters are important to reproduce the dynamics and final size of an epidemic: the serial interval $SI$, which is 
the interval between two subsequent infections, and the basic reproductive number 
$R_0$ representing the number of newly infected caused by an infecteds in a fully susceptible population.

For SARS-CoV-2, the serial interval was estimated to be in range $6.5-8.2$. We draw uniformly from this range, and we solve
