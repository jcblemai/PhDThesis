\begin{fullwidth}
\chapter{Scenario planning tools and reports for the COVID-19 pandemic}
\label{sec:covid-pipeline-reports}
  \end{fullwidth}s
The ongoing Coronavirus disease 2019 (COVID-19, previously nCov) pandemic originated in Wuhan, China end of 2019. The pathogen is the severe acute respiratory syndrome coronavirus SARS-CoV-2. It is one of the deadliest pandemic in history, and the response has mobilized scientist, public-health and medical professionals and politics in an unprecented way. This chapter presents some modeling works for the response to COVID-19. As state of knowledge on the SARS-CoV-2 and COVID-19 was and still is quickly expanding, the present chapter and the next hightlight the challenges of dealing with such uncertainties. For the same reason, we do not endeavour a litterature review\sidenote[][5\baselineskip]{That would be ambitious, with a bit less than 170'000 peer-reviewed papers and many more preprints, website and reports. Estimation from:\fullcite{COVID-19OpenAccessProject:LivingEvidenceCOVID19:2020}} of the current understanding of COVID-19 and modeling studies. 

We focus on details dealing with the uncertainties that caraterize disease emergences.


\section{The COVID Scenario Pipeline}
The COVID Scenario pipeline\sidenote[][5\baselineskip]{\url{github.com/HopkinsIDD/COVIDScenarioPipeline}} has been develop by the Johns Hopkins University Infectious Disease Dynamics COVID-19 Working Group's. It is a flexible modeling framework that projects epidemic trajectories and healthcare impacts under different suites of interventions in order to aid in scenario planning. The model is generic enough to be applied to different spatial scales given shapefiles, population data, and COVID-19 confirmed case data. There are multiple components to the pipeline, which maxy be characterized as follows: 1) epidemic seeding; 2) disease transmission and non-pharmaceutical intervention scenarios; 3) calculation of health outcomes (hospital and ICU admissions and bed use, ventilator use, and deaths); and 4) summarization of model outputs.

Other parts of the pipeline not described in this thesis (airport importation, report generation, config writing, inference, analysis, submission)\footnote{see \url{github.com/HopkinsIDD/COVIDScenarioPipeline} and \url{github.com/HopkinsIDD/covidImportation}}. 1. Intro Unraveling of the epidemic since January: from importation models to intervention model

A previous version of the ever evolving pipeline has been described in:
\longfullcite{Lemaitre:ScenarioModelingPipeline:2021}.
\begin{figure*}[!htb]
    \centering
...
The transmission model of SARS-CoV-2 is configurable to any  along an arbitrary transition graph, to allow for multi-strain, age-stratified models, and the pipeline is able to infer parameters values from data. However, the initial model built for scenario planning was much simpler. The initial model was hard-coded, and was a simple spatial $S\longrightarrow E \longrightarrow I \longrightarrow R$ model. In practice, there is $k = 3$ compartments of infected, $I_1, I_2, I_3$ in order to have an infectious period shaped as Erlang distribution.
...
\caption{Transitions rate parameters for the pipeline}
     \label{tab:survpars}
\end{table}
Two meta-parameters are important in reproducing the dynamics and final size of an epidemic: the serial interval $SI$, which is 
...
The serial interval was estimated to be in range $6.5-8.2$.,We draw uniformly from this range, and we solve
...
\begin{equation}
	\beta_i'(t) =  \beta_i(t) \cdot  \prod_k \left(1-r_k(t) \right) \ref{eq:npi}% _{i=1}^{N_{\text{mod(t)}}
\end{equation}
Several interventions templates are provided, and the copounding  of interventions is with either multiplicative (as in Equation \ref{eq:npi}) or additive copounding depending on the parameters. 
...
$O[t+\Delta:t+\Delta+D]\leftarrow O[t+\Delta:t+\Delta+D] + \text{Binom}(S[t], p_{O|S})$\;
 }
 }
...
% pagecommand adds page numbers,  need package pdfpages
 ...
 \includepdf[pages=-, pagecommand={\thispagestyle{empty}}]{fig_pipeline/planning.pdf}
