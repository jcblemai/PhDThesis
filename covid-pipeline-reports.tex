...
The user may specify health outcome probabilities and delays, conditional on the flows described above. For use as default values, tables of parameter values derived from a literature review of \textsc{covid}-19 health outcomes are provided in the supplementary information of the postprint.
...
All components and settings for simulations from the COVID  Scenario Pipeline model are specified in an easily-modifiable YAML configuration file. Different options are described in detail in the postprint and the GitHub repository.

\subsection{Recent developments}
This chapter solely focuses on a perspective from June 2020. The COVID Scenario Pipeline transmission and outcomes model structure were hard-coded to simple models, and yet were able to capture early \textsc{covid}-19 dynamics without inference. It demonstrates how simple models can capture epidemiological dynamics by focusing on a few important aspect of disease transmission. Provided uncertaintes on serial interval and $R_0$ and how models allow one to exploit the current state of knowlegde to reason about an uncertain future. 

As mentioned in margin notes, most of the assumptions described in this chapter have been relaxed as of today, which allows , + focus on performance. and helpers to write configuration files (xx lines).

%TODO WRITE this page.
\section{Results}
The COVID Scenario Pipeline has been used to support several partners including the state of California and the national US response, with regular reports of diverse nature. While these not public, current ouputs from the COVID Scenario Pipeline are shared through multi-modeling experiments.

\paragraph{covid-19 forecast hub} Every week dozens of modeling teams from across the globe submit forecasts of the trajectory of the COVID-19 pandemic in the US to our forecast data repository. In collaboration with the US CDC, we take these data and build a single ensemble forecast which is later analyzed by, and communicated to the general public by CDC.

\paragraph{Scenario Modeling Hub}


\subsection{Example of scenario planning report for Canton de Vaud}
A report produced on April 9, 2020 for Canton de Vaud main hopital, CHUV, is presented here. It is different from standart pipeline report, but it is nonetheless interesting to read. The inference module of the pipeline did not exist yet, so scenario projects were generated using a simple particle filter like methods\footnote{This is a collaboration between many different actors further analysis and algorithm developed with Dr. Perez-Saez and Prof Jacques Fellay.}
